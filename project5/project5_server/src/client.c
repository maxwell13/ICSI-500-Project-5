...
int clientMain(int argc, char *argv[])
{
    ...
    printf("Please enter the command then testing file followed by a space: ");
    bzero(buffer,256);

    //gets input fro stdin
    fgets(buffer,255,stdin);

    // memcpy(buffer, "toUpper testingFile.txt ",24 );
    // memcpy(buffer, "count F testingFile.txt",23 );
